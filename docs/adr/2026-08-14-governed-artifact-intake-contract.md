...
Every processing transition requires the current lease token and a single-row compare-and-swap. Before provider fetch or vision, a retry checks the canonical finalization identity and repairs D1 from authoritative Neon when canonical capture already succeeded. Adapter extraction needed across a crash is kept only in a bounded TMK-encrypted recovery envelope in managed R2; D1 and the queue retain only opaque operation state. Stale workers cannot finalize, retry, or fail the job. A queue delivery that finds an active processing lease is retried after the bounded remaining lease duration and is never acknowledged as already handled. The expiry reaper also attempts canonical-first recovery before it can fail an abandoned processing job. An already canonical capture is recovered to delivery and can never be converted into a failed job or false failure reply.
