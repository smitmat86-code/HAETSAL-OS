...
Unknown or unreadable envelopes, missing R2 references, incomplete object evidence, any non-singleton authoritative Neon reference set, duplicate D1 references, D1-only references, multi-owner references, multiple legacy source keys in one capture, and D1/Neon ownership disagreements are ambiguous and deletion-ineligible. Only an object with zero Neon and zero D1 references can begin as an orphan. With no explicit object-level replacement provenance in the current schema, a legacy source is “already migrated” only when the capture has exactly one eligible legacy source row and exactly one managed primary source selected by both capture and document; a second legacy key, duplicate managed-primary candidate, derivative, or unrelated artifact does not qualify. Changing any exact target or approved category, even while preserving aggregate counts and byte totals, changes the contract, and an executor rejects a matching digest if the named categories are absent or different.
