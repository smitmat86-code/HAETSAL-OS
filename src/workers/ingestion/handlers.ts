...
export async function handleSmsInbound(
  ...
  const channel: 'sms' | 'sendblue' | 'telegram' =
    rawChannel === 'sendblue' ? 'sendblue' : rawChannel === 'telegram' ? 'telegram' : 'sms'
  const metadata: Record<string, unknown> = channel === 'telegram'
    ? { telegram_chat_id: from }
    : { from_phone: from }
  await retainContent(
    {
      tenantId, source: channel, content: text, occurredAt, provenance: channel,
      metadata,
