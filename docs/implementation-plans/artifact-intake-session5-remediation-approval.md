...
Unknown or unreadable envelopes, missing R2 references, incomplete object evidence, any non-singleton authoritative Neon reference set, duplicate D1 references, D1-only references, multi-owner references, multiple legacy artifacts in one capture, D1/Neon ownership or role disagreements, and derivative, missing, or unknown legacy roles are ambiguous and deletion-ineligible. Neon role is explicit; D1 proves `source` only through matching capture/document primary pointers and otherwise reports unknown role evidence. Only an object with zero Neon and zero D1 references can begin as an orphan. A legacy source is “already migrated” only when the capture has exactly one role=`source` legacy artifact and exactly one managed role=`source` primary selected by both capture and document; a second legacy artifact, mixed source/derivative rows, duplicate managed-primary candidate, or unrelated artifact fails closed. Changing any exact target or approved category, even while preserving aggregate counts and byte totals, changes the contract, and an executor rejects a matching digest if the named categories are absent or different.

The query boundary is executable, not string-inspected. `LEGACY_D1_CANONICAL_REFERENCES_SQL` runs against the migrated local D1 test schema. `LEGACY_MANAGED_REPLACEMENT_CANDIDATES_SQL` executes and `EXPLAIN`s against an ephemeral, non-production PGlite PostgreSQL schema. Fixtures prove the sole legitimate legacy source plus unique managed primary qualifies, while derivative, mixed source/derivative, missing/conflicting pointers, duplicate legacy/managed candidates, and a managed artifact owned by another capture do not. No production inventory data is used.
